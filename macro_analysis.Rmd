...
macro.sample <- macro.data %>% 
  group_by(sampleID) %>% 
  dplyr::summarise (total.num = sum(number, na.rm = TRUE))

q.area <- sampling.sum %>% select(SampleID, q.area)

macro.sample <- left_join(macro.sample, q.area, by = "SampleID")
...
###calculate the ffg density 
macro.ffg <-macro.ffg %>% 
  mutate(ffg.den=num.ffg/q.area)


ggplot(ID.ffg, aes(x=Year,y=relab, fill = FFG))+
