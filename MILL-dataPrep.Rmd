...
```{r join macro waterQ and quadrat fall 2019}

#filter waterQ
waterQ.F19 <- waterQ %>% 
  filter(year == "2019", sampling == "macros", season == "Fall") %>% 
...
abiotic.F19 <- left_join (quad.F19, waterQ.F19)
  

##Join macro data and waterQ
macro.abio.F19 <- macro.data %>% 
  filter(year == "2019", season == "Fall") %>% 
  left_join(., abiotic.F19) 
...
macro.abio <- bind_rows(macro.abio.S19, macro.abio.F19,
                        macro.abio.F18, macro.abio.S18) %>% 
  dplyr::select(-BvA, -sampling, -aTemp, -num_quads, -num_w)
