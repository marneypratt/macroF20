...
#Save this data file
write.csv(BARC_fieldData, "NEONdata/BARC/BARC_fieldData.csv")


# Make the observation table.
# start with inv_taxonomyProcessed
BARC.macro <- inv_taxonomyProcessed %>% 
...
BARC.macro <- left_join(BARC.macro, neon.mill.taxa2, by="acceptedTaxonID")


# save macro data file for this site
write.csv(BARC.macro, "NEONdata/BARC/BARC.macro.csv", 
          row.names = FALSE)
...
## Import NEON Macroinvertebrate Data for BARC Site
...
BARC.macro <- read_csv("NEONdata/BARC/BARC.macro.csv")
...
# note that I relabeled the siteID because this was the closest where precip was measured
precip <- precip_daily %>%
                group_by(year) %>%
                arrange(year, yday) %>%
...
write.csv(precip, "NEONdata/BARC/BARC.precip.csv",
row.names = FALSE)
...
BARC.precip<- read_csv("NEONdata/BARC/BARC.precip.csv",
...
```{r join NEON macros and environmental vars for BARC}


# select columns needed
BARC.precip.r <- BARC.precip %>% dplyr::select(date, mon.precip)

# join precip to macro data
BARC.macro.precip <- left_join(BARC.macro, BARC.precip.r, by = "date") %>% 
  filter(!is.na(mon.precip))
...
write.csv(BARC.macro.precip, "preppedData/BARC.macro.precip.csv", 
          row.names = FALSE)
